...
close all
clc
filepath = '/home/abdulrah/catkin_ws/src/apple_all_strategies/*.bag';
FileList = dir('/home/abdulrah/catkin_ws/src/apple_all_strategies/*.bag');
N = size(FileList,1);
dir = FileList.folder;
for k = 1:N
...
hold all
legend ('Chopping','Pressing\_and\_slicing','Sawing','Slicing')
figure(2);
plot(raw_time - raw_time(1), position(3,:));
legend ({'Chopping','Pressing\_and\_slicing','Sawing','Slicing'},'Location','southeast');
xlim([30 75]);
xlabel('time [S]');
ylabel('EEpose Z coordinate [m]');
hold all
